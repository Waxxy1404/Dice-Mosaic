...
def validate_square_image(image: np.ndarray) -> None:
    ...
    if width != height:
        raise ValueError(
            "A imagem enviada para o pipeline precisa ser quadrada. "
            "No Streamlit, selecione uma região quadrada antes de gerar o mosaico."
        )


def prepare_square_image(
    image_bgr: np.ndarray,
    target_size: int = TARGET_CROP_SIZE,
) -> np.ndarray:
    """Validate a square image and resize it to the target processing size."""
    ...
    interpolation = cv2.INTER_AREA if width > target_size else cv2.INTER_CUBIC
    return cv2.resize(
        image_bgr,
...
def build_mosaic(image_bgr: np.ndarray, config: MosaicConfig) -> MosaicResult:
    image_bgr = prepare_square_image(image_bgr, TARGET_CROP_SIZE)
    ...
    for row in range(rows):
        ...
        for col in range(cols):
            ...
            if rotations is None:
                ...
                row_vals.append(_format_face_cell(face, int(rotations[row, col])))
...
def matrix_to_json(matrix: np.ndarray, rotations: np.ndarray | None = None) -> str:
    payload = {
        ...
        "source_crop_size_px": TARGET_CROP_SIZE,
